...
AreaBLandscapeSensible::AreaBLandscapeSensible(uint32_t x, uint32_t y, uint32_t playerId, std::shared_ptr<Collection<Unit>> units, std::shared_ptr<Collection<GO>> go, uint32_t mapW, uint32_t mapH) :
...
std::shared_ptr<Collection<GO>> AreaBLandscapeSensible::getGO() {
    return this->go;
}
...
std::map<std::tuple<uint32_t, uint32_t>, uint32_t> AreaBLandscapeSensible::getAvailable() {
    ...
    for (uint32_t i = 0; i < this->go->size(); i = i + 1) {
        ...
        if ((!this->ignoreLowObstacles() and o->isLowObstacle(this->getPlayerId())) or
            (!this->ignoreHighObstacles() and o->isHighObstacle(this->getPlayerId())) or
            (!this->ignoreUltraHighObstacles() and o->isUltraHighObstacle(this->getPlayerId()))) {
