	...
	for (; ;) {
        ...
        if (!this->startGameMap.empty()) {
            try {
                ...
                std::shared_ptr<WindowButton> w = std::make_shared<WindowButton>(*Locales::get()->get("failed_to_load_map"), *Locales::get()->get("OK"), clickEvent);
...
void Menu::init(uint32_t windowW, uint32_t windowH) {
    ...
    std::shared_ptr<WindowButton> licenseWindow = std::make_shared<WindowButton>(*Locales::get()->get("license"), *Locales::get()->get("close"), clickEvent, 600, 400);
