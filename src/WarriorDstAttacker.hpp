...
class WarriorDstAttacker : public WarriorAttacker {
public:
    WarriorDstAttacker();
    WarriorDstAttacker(uint32_t x, uint32_t y, uint32_t playerId);
protected:
    virtual uint32_t getAttackRadius() const = 0;
...
BOOST_CLASS_EXPORT_KEY(WarriorDstAttacker)