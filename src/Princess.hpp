...
class Princess : public WarriorLeader {
public:
    Princess();
    Princess(uint32_t x, uint32_t y, uint32_t playerId);
    ...
    std::string getStartInspireSoundName() const override;
    uint32_t getMaxHP() const override;
    Defence getBaseDefence() const override;
    uint32_t getInspireRadius() const override;
...
private:
    ...
    uint32_t getInspireAnimationsNumberInSet() const override;
    uint32_t getMovementPoints() const override;
    uint32_t getPopulation() const override;

    friend class boost::serialization::access;
    template<class Archive> void serialize(Archive &ar, const unsigned int version) {
        ar & boost::serialization::base_object<WarriorLeader>(*this);
    }
};


BOOST_CLASS_EXPORT_KEY(Princess)