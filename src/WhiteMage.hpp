...
class WhiteMage : public WarriorDstSingleAttacker {
public:
    WhiteMage();
    WhiteMage(uint32_t x, uint32_t y, uint32_t playerId);
    ...
    uint32_t getAttackRadius() const override;
private:
    ...
    Events eventAfterAnimation(uint32_t targetX, uint32_t targetY) override;

    friend class boost::serialization::access;
    template<class Archive> void serialize(Archive &ar, const unsigned int version) {
        ar & boost::serialization::base_object<WarriorDstSingleAttacker>(*this);
    }
};


BOOST_CLASS_EXPORT_KEY(WhiteMage)