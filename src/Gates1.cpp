...
std::shared_ptr<Building>  Gates1::createSameTypeBuilding() const {
    return std::make_shared<Gates1>(this->getX(), this->getY(), this->getPlayerId());
}
...
StringLcl Gates1::getDescription() const {
    return StringLcl("{gates1_description}");
}
